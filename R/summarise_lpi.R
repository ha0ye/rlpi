...
summarise_lpi <- function(infile) {
  ...
  for (FileNo in 1:NoFiles) {
    ...
    grDevices::pdf(file=paste(filename, ".summary.pdf", sep=""), width=16, height=6)
    ...
    utils::write.table(npop_year, file=paste(filename, ".npop_year.txt", sep=""), row.names=FALSE)
    ...
    utils::write.table(nsp_year, file=paste(filename, ".nsp_year.txt", sep=""), row.names=FALSE)

    nyear_sp = plyr::ddply(Data, 'Binomial', plyr::summarise, nyear = length(unique(year)))
    colnames(nyear_sp) <- c("species", "nyear")
    p3 <- ggplot2::ggplot(nyear_sp, ggplot2::aes(x = stats::reorder(species, nyear), y = nyear)) +
    ...
    utils::write.table(nyear_sp[order(nyear_sp$nyear), ], file=paste(filename, ".nyear_sp.txt", sep=""), row.names=FALSE)

    nyear_pop = plyr::ddply(Data, c('ID', 'Binomial'), plyr::summarise, nyear = length(unique(year)))
    colnames(nyear_pop) <- c("pop", "binomial", "nyear")
    p4 <- ggplot2::ggplot(nyear_pop, ggplot2::aes(x = stats::reorder(pop, nyear), y = nyear)) +
      ggplot2::geom_point(size=2) +
    ...
    utils::write.table(nyear_pop[order(nyear_pop$nyear), ], file=paste(filename, ".nyear_pop.txt", sep=""), row.names=FALSE)
    ...
    p5 <- ggplot2::ggplot(df, ggplot2::aes(colour="black")) +
      ggplot2::geom_segment(ggplot2::aes(x=minyear, xend=maxyear, y=stats::reorder(Binomial, duration), yend=stats::reorder(Binomial, duration)), size=0.5, colour="black") +
      ggplot2::xlab("Duration")
    print(p5)

    grDevices::dev.off()
  }
  utils::write.table(summary_data, file=paste(infile, ".summary.csv", sep=""), sep=",", row.names=FALSE)
